...
def check_and_assign(tri_arr1, tri_arr_temp, neigh_arr, i , j, curr_update_index):
    ...
    if common_side(tri_arr1[i], tri_arr_temp[j], 0, 1):
        neigh_arr[i][curr_update_index[i].astype(int)] = j
        curr_update_index[i] = curr_update_index[i] + 1
        neigh_arr[j][curr_update_index[j].astype(int)] = i
        curr_update_index[j] = curr_update_index[j] + 1
...
def assign_neigh_index(tri_arr1, num_tri, neigh_arr):
    ...
    curr_update_index = np.zeros((num_tri, 1))
